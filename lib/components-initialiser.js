...
export class ComponentsInitialiser {
  ...
  initialise() {
    ...
    $('[data-collapse-target]').off('click').on('click', (evt) => {
      ...
      // Widget which only shows
      $('[data-checkbox-collapse-target]').off('click').on('click', (evt) => {
          // console.log('Collapse click:', evt);
          var $el = $(evt.currentTarget);
          var selector = $el.attr('data-checkbox-collapse-target');
          var $target = $(selector);
          // var $caption = $el.find('.mdc-typography--caption');
          var $icon = $el.find('.far');
      ...
      $('.mdc-form-field input[type="checkbox"]').on('change', function(evt) {
        $(evt.currentTarget).parents('.mdc-form-field').toggleClass('checked', evt.currentTarget.checked); }).each(function(i, o) {
