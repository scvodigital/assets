...
export class ComponentsInitialiser {
  ...
  initialise() {
    ...
    $('[data-collapse-target]').off('click').on('click', (evt) => {
      ...
      // Collapsibles
      $('[data-checkbox-hide-target]').off('click').on('click', (evt) => {
          // // console.log('Collapse click:', evt);
          // var $el = $(evt.currentTarget);
          // var selector = $el.attr('data-checkbox-hide-target');
          // var $target = $(selector);
          // // var $caption = $el.find('.mdc-typography--caption');
          // var $icon = $el.find('.far');
          // if ($target.hasClass('show_all_checkboxes')) {
      ...
      // List addition
      $('[data-list-target]').change((evt) => {
          // var $el = $(evt.currentTarget);
          // var selector = $el.attr('data-list-target');
          // var content = $el.attr('data-list-content');
          // var listTag = $el.attr('data-list-tag') || "li";
          // if ($el[0].checked){
          //   var $target = $(selector);
          //   $target.append( "<" + listTag + ">" + content + "</" + listTag + ">");
          // } else {
          //     $(selector + " > " + listTag).each(function () {
      ...
      // Chip addition
      $('[data-chip-checkbox-target]').change((evt) => {
          var $el = $(evt.currentTarget);
          var id = $el.attr('id');
          if (!id) throw new Error("ID required for chip addition checkbox");
          var selector = $el.attr('data-chip-checkbox-target');
          var content = $el.data('list-content');
          if ($el[0].checked){
              var $target = $(selector);
              var chipElement = $('<div></div>');
              chipElement.attr('id', id + "_chip");
              chipElement.append(content.data);
              $target.append(chipElement);
              var removeElement = chipElement.find(".remove_element");
              if (removeElement) removeElement.on("click", () => {
                  chipElement.remove();
