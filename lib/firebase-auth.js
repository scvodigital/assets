...
export class Auth {
  ...
  async changePassword(password, passwordConfirm, oldPassword) {
    try {
      ...
      if (oldPassword) {
        try {
          console.log('Given old password so reauthenticating');
          currentUser = await this.currentUser();
          console.log('Got current user', currentUser, 'Creating credential');
          const authCred = firebase.auth.EmailAuthProvider.credential(currentUser.email, oldPassword);
          console.log('Created credential', authCred, 'Reauthenticating');
          const userCred = await currentUser.reauthenticateWithCredential(authCred);
          console.log('Reauthenticated', userCred);
        } catch(err) {
